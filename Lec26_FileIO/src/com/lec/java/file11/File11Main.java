...
public class File11Main {
	public static void main(String[] args) {
		...
		try (FileWriter fw = new FileWriter(dst); 
			FileReader fr = new FileReader(src);

		) {
			String str = "안녕하세요"; //한글 5글자 1글자당 3바이트
 			char [] charArr = {'J','A','V','A'}; //한바이트 총 19바이트
			
			fw.write(str); //저장은 시스템 인코딩 상태에 따라 저장됨.
							//UTF-8 인코딩의 경우 ,한글은 한글자당 3byte, 영문은1byte로 저장
			fw.write(charArr);
			fw.flush(); //write() 호출후에는 flush()하여 출력버퍼의 데이터가 완전히 출력되도록한다.
			
			//읽을때는 캐릭터형 버퍼에 담아야한다
			//읽기
			char []buff =new char[100]; //byte가아니라 char 글자단위로읽는다 
			int charsRead = 0; // 읽어들인 문자의 개수 
			charsRead = fr.read(buff); //몇글자 읽어들엿는지 리턴
